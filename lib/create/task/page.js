...
var Page = Base.extend({
  ...
  talk: function (cb) {
    ...
    var conf = this.conf;
    if (conf.remote === 'tencent') {
      conf.isTencent = true;
    } else if (conf.remote === 'jd') {
      conf.isTencent = false;
    }
    ...
    if (typeof conf.isTencent !== 'boolean') {
      prompts.push({
        type: 'confirm',
        name: 'isTencent',
        message: '是否腾讯域下',
  ...
  write: function (cb) {
    ...
    this.template(this.moduleConf.tmpId , 'page' , 'page.html', 'page/' + pageName + '/' + pageName + '.html', this, {
      delimiter: '$'
    });
    ...
    this.copy(this.moduleConf.tmpId , 'page' , 'page.css', cssFileName);
    this.copy(this.moduleConf.tmpId , 'page' , 'page.js', 'page/' + pageName + '/' + pageName + '.js');
    this.copy(this.moduleConf.tmpId , 'page' , 'page.json', 'page/' + pageName + '/' + pageName + '.json');
