...
var Widget = Base.extend({
   ...
   * @param {Function} cb - 创建完后的回调
   */
  loadRemote: function (cb) {
    var that = this;
    var wname = that.conf.widgetName.split('~');
    var tarpath = 'widget/' + that.conf.widgetName;
    var req = request.get(setting.remote_widget_url + '?cate=' + wname[0] + '&filename=' + wname[1]);
    req.pipe(unzip.Extract({ path: tarpath })
      .on('close', function () {
        if(Util.existsSync(path.join(tarpath,'index.html'))){
          fs.renameSync(path.join(tarpath,'index.html'),path.join(tarpath,that.conf.widgetName + '.html'));
        }
        if (Util.existsSync(path.join(tarpath,'index.scss'))) {
          fs.renameSync(path.join(tarpath,'index.scss'),path.join(tarpath,that.conf.widgetName + '.scss'));         
        }
        if (Util.existsSync(path.join(tarpath,'index.css'))) {
          fs.renameSync(path.join(tarpath,'index.css'),path.join(tarpath,that.conf.widgetName + '.css'));         
        }
        if (Util.existsSync(path.join(tarpath,'index.less'))) {
          fs.renameSync(path.join(tarpath,'index.less'),path.join(tarpath,that.conf.widgetName + '.less'));         
        }
        if (Util.existsSync(path.join(tarpath,'index.js'))) {
          fs.renameSync(path.join(tarpath,'index.js'),path.join(tarpath,that.conf.widgetName + '.js'));         
        }
