...
var Widget = Base.extend({
  ...
  loadremote: function(cb){
    ...
    request.get(setting.remote_widget_url + '?cate=' + wname[0] + '&filename=' + wname[1] )

    .pipe(unzip.Extract({ path: tarpath }).on('close',function(){
      if(fs.existsSync(path.join(tarpath,'index.html')) && fs.existsSync(path.join(tarpath,'index.scss'))){
          fs.renameSync(path.join(tarpath,'index.html'),path.join(tarpath,that.conf.widgetName + '.html'))
          if(fs.existsSync(path.join(tarpath,'index.scss'))){
            fs.renameSync(path.join(tarpath,'index.scss'),path.join(tarpath,that.conf.widgetName + '.scss'))
          }else{
            fs.renameSync(path.join(tarpath,'index.css'),path.join(tarpath,that.conf.widgetName + '.scss'))
          }
          
        }
        console.log( 'Created ' + ( that.conf.widgetName || ' in current folder' ) );
    }))
    .on('error', function(){
        console.log('获取远程组件失败，请检查网络或模板名！错误代码：' + arguments[0]);
    })
