...
var Util = {
  regexps: {
    ...
    // media
    media: /\.(jpeg|jpg|gif|png|webp|mp3|mp4|oog|wav|eot|svg|ttf|woff)/
  },
  homedir: function () {
    function homedir () {
      var env = process.env;
  ...
  getHashName: function (id, mapJson) {
    ...
    if (id.indexOf(path.sep) < 0) {
      return revByType[name] + (nameHash ? ('#' + nameHash) : '');
    }
  ...
  getConfig: function () {
    var configPath = path.join(path.resolve(__dirname, '../../'), '.config.json');
    var config = {};
  ...
  setConfig: function (config) {
    if (typeof config === 'object') {
      fs.writeFileSync(path.join(path.resolve(__dirname, '../../'), '.config.json'), JSON.stringify(config, null, 2));
    }
  },

  getSetting: function () {
    var settingPath = path.join(path.resolve(__dirname, '../../'), '.setting.json');
    var setting = {};
  ...
  setSetting: function (setting) {
    if (typeof setting === 'object') {
      fs.writeFileSync(path.join(path.resolve(__dirname, '../../'), '.setting.json'), JSON.stringify(setting, null, 2));
    }
  },
