...
var Util = {
  ...
  regexps: {
    ...
    doubleBraceInterpolate: /{{\s?[\w\s]*\s?}}/g
  },
  ...
  getHashName: function (id, mapJson) {
    ...
    var splitAfter = null;
    if (splitIndex >= 0) {
      splitAfter = id.substr(splitIndex);
      id = id.substr(0, splitIndex);
    }
    if (!revByType || !revByType[id]) {
      return id;
    }

    return idPrefix + revByType[id] + (splitAfter ? splitAfter : '');
  },
  ...
  getHashNameServer: function (id, mapJson) {
    ...
    return revByType[id] + (splitAfter ? splitAfter : '');
  },
