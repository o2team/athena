...
var Util = {
  ...
  processHtml: function (contents, cb) {
    var reg = /(<script(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/script\s*>|$)|(<style(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/style\s*>|$)|<(img|embed|audio|link|video|object|source)\s+[\s\S]*?["'\s\w\/\-](?:>|$)|__uri\(\s*([\s\S]*?)\)|<!--uri\[([^\]]+)\]-->|__inline\(\s*([\s\S]*?)\)|<!--inline\[([^\]]+)\]-->/ig;
    contents = contents.replace(reg, function (m, $1, $2, $3, $4, $5, $6, $7, $8, $9) {
      if ($1) { //<script>
        var embed = '';
  ...
  /**
   * Inspect anything.
   * @param {...*} args - Things you want to inspect.
   */
