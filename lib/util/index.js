...
var Util = {
  ...
  getHashName: function (id, mapJson) {
    ...
    var splitAfter = null;
    if (splitIndex >= 0) {
      splitAfter = name.substr(splitIndex);
      name = name.substr(0, splitIndex);
    }
    ...
    if (id.indexOf('/') < 0) {
      return revByType[name] + (splitAfter ? splitAfter : '');
    }
    return dirname + '/' + revByType[name] + (splitAfter ? splitAfter : '');
  },
