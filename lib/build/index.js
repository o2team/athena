...
function deploy (app, mod) {
  ...
  if (conf.buildType === BUILD_APP) {
    ...
    del.sync(path.join(conf.modulePath, 'dist'));
    var deployPromise = taskList.deploy($, conf.appConf, conf.moduleConf);
    return buildSingleModule(app, mod, conf).then(deployPromise.bind(null, mod, conf.modulePath, conf.appPath)).catch(function (e) {
      console.log(e.stack);
    });;
  }
}
...
function publish (app, mod) {
  ...
  if (conf.buildType === BUILD_APP) {
    ...
    var publishPromise = taskList.publish($, conf.appConf, conf.moduleConf);
    return buildSingleModule(app, mod, conf).then(publishPromise.bind(null, mod, conf.modulePath, conf.appPath)).catch(function (e) {
      console.log(e.stack);
    });;
  }
}
