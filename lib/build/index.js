...
function build (app, mod, args) {
  ...
  if (conf.buildType === BUILD_APP) {
    ...
    return promsies.reduce(function (prev, curr) {
      ...
      .then(function () {
        if (args.pack || args.compress) {
          ...
          var zipName = conf.appConf.app + '.zip';
    ...
    del.sync(path.join(conf.appPath, '.temp', conf.moduleConf.module), { force: true });
    if (args.isSimple) {
    ...
    build(app, mod, _.assign({ isSimple: !isDist }, args)).then(function () {
      ...
      vfs.watch([
        ...
        servePagePromise(mod, modulePath, conf.appPath)
          .then(function () {
            maltose.reload([path.join(conf.appPath, '.temp', moduleFolder, path.basename(fPath))]);
          });
      });
      ...
      vfs.watch([
        ...
        serveTaskList.reduce(function (prev, curr, index) {
          ...
          .then(function () {
            maltose.reload([path.join(conf.appPath, '.temp', moduleFolder, path.basename(currentUrl))]);
          })
          .catch(function (e) {
            if (e) {
              console.log(e.plugin);
      ...
      vfs.watch([
        ...
        copyFilePromise(mod, modulePath, conf.appPath)
          ...
          .then(function () {
            maltose.reload([path.join(conf.appPath, '.temp', moduleFolder, pageName + '.html')]);
          });
      });
      ...
      .then(function () {
        ...
        vfs.watch([
          ...
          servePagePromise(mod, conf.modulePath, conf.appPath)
            .then(function () {
              maltose.reload([path.join(conf.appPath, '.temp', mod, path.basename(fPath))]);
            });
        });
        ...
        vfs.watch([
          ...
          copyFilePromise(mod, conf.modulePath, conf.appPath)
            ...
            .then(function () {
              maltose.reload([path.join(conf.appPath, '.temp', mod, path.basename(currentUrl))]);
            });
        });
        ...
        vfs.watch([
          ...
          copyFilePromise(mod, conf.modulePath, conf.appPath)
            ...
            .then(function () {
              maltose.reload([path.join(conf.appPath, '.temp', mod, pageName + '.html')]);
            });
        });
