...
function buildSingleModuleServer (app, mod, conf, args) {
  ...
  var publishPrepareServerPromise = taskList.publish_prepare_server($, conf.appConf, conf.moduleConf, args);
  var replaceStaticServerPromise = taskList.replace_static_server($, conf.appConf, conf.moduleConf, args)
  var resourceConcatServerPromise = taskList.resource_concat_server($, conf.appConf, conf.moduleConf, args);
  var fetchCommonPromise = taskList.fetch_common($, conf.appConf, conf.moduleConf, args);
  var webpackPromise = taskList.webpack($, conf.appConf, conf.moduleConf, args)
  ...
  return allPromise(mod, conf.modulePath, conf.appPath)
    ...
    .then(function () {
      if (args.isPublish) {
        return publishPrepareServerPromise(mod, conf.modulePath, conf.appPath)
      }
      args.remoteName = (args && args.remote) ? args.remote : 'local'
      return Promise.resolve()
    })
    .then(replaceStaticServerPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(webpackPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(function () {
      // 是否启用md5
      return new Promise(function (resolve, reject) {
        var support = conf.moduleConf.support;
...
function buildSingleModuleSimpleServer (app, mod, conf, args) {
  ...
  var stylesServerPromise = taskList.styles_server($, conf.appConf, conf.moduleConf, args);
  var injectServerPromise = taskList.inject_server($, conf.appConf, conf.moduleConf, args);
  var replaceStaticServer = taskList.replace_static_server($, conf.appConf, conf.moduleConf, args)
  ...
  return allPromise(mod, conf.modulePath, conf.appPath)
    ...
    .then(stylesServerPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(injectServerPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(replaceStaticServer.bind(null, mod, conf.modulePath, conf.appPath))
...
function build (app, mod, args) {
  ...
  var webpackDllPromise = taskList.webpack_lib($, conf.appConf, conf.moduleConf, args)
  webpackDllPromise(mod, conf.modulePath, conf.appPath)
    .then(function () {
      buildCheck(app, mod, conf, args, buildSingleModuleServer)
    })
}
...
function watchExtraDirs (mod, conf) {
  const appConf = conf.appConf
  const onWatchDirs = appConf.onWatchDirs || ['reducers', 'actions', 'store', 'constants']
  onWatchDirs.map(function (item) {
    watch([
      '*/' + item + '/**'
    ], function (ev) {
      var fPath = ev.path;
      ...
      var copyFilePromise = taskList.copy_file($, appConf, moduleConf, {
        src: fPath,
        dest: path.join(modulePath, 'dist', '_', item)
      });
      var copyFileToStaticPromise = taskList.copy_file($, appConf, moduleConf, {
        src: fPath,
        dest: path.join(modulePath, 'dist', '_static', item)
      });
      var copyFileToOutputPromise = taskList.copy_file($, appConf, moduleConf, {
        src: fPath,
        dest: path.join(modulePath, 'dist', 'output', 's', item)
      });
      copyFilePromise(mod, modulePath, conf.appPath)
        .then(copyFileToStaticPromise.bind(null, mod, modulePath, conf.appPath))
        .then(copyFileToOutputPromise.bind(null, mod, modulePath, conf.appPath))
    })
  })
}
...
function serveServer (conf, app, mod, args) {
  ...
  var appConf = conf.appConf;
  if (conf.buildType === BUILD_APP) {
    return build(app, mod, _.assign({ isSimple: !isDist }, args)).then(function (moduleList) {
      // 执行webpack处理
      if (!moduleList || !moduleList.length) {
        throw new Error('没有需要经过webpack编译的模块！')
      }
      ...
      const webpackConfig = []
      const port = 8080
      const modules = Array.isArray(conf.modules) ? conf.modules : ['*']
      const modulesGlobPrefix = modules.length > 1 ? `{${modules.join(',')}}` : modules[0]
      const outputPaths = moduleList.map(function (mod) {
        const modulePath = path.join(conf.appPath, mod)
        const moduleConf = require(path.join(modulePath, 'module-conf.js'))
        const webpackDevConfig = require('./webpack.dev.config')(conf.appPath, modulePath, appConf)
        ...
        const webpackBaseConfig = require('./webpack.base.config')(conf.appPath, modulePath, appConf)
        ...
        allPages.forEach(function (item) {
          ...
          const jsPath = path.join(moduleOutputPath, 's', 'page', pageName, pageName + '.js')
          entry[pageName] = [
        ...
        const staticJSFolder = path.join(moduleOutputPath, 's', 'static', 'js')
        ...
        fs.ensureDirSync(path.join(conf.appPath, '.temp', appConf.app, mod))
        webpackConfig.push(webpackMerge(webpackDevConfig, webpackBaseConfig, {
          entry: entry,
          output: {
            path: path.join(conf.appPath, '.temp', appConf.app, mod),
          ...
          resolve: {
            alias: (function () {
              ...
              refModuleList.forEach(function (item) {
                aliasObj[`@${item}`] = path.join(conf.appPath, item, 'dist', 'output', 's')
              })
              return aliasObj
            })()
          }
      ...
      const publicPath = '/' + appConf.app + '/'
      const server = new WebpackDevServer(compiler, {
        contentBase: path.join(conf.appPath, '.temp'),
        watchContentBase: true,
        publicPath: moduleList.length > 1 ? publicPath : publicPath + moduleList[0] + '/',
        hot: true,
        compress: false,
      ...
      server.middleware.waitUntilValid(function () {
        var serveUrl = Util.urlJoin('http://localhost:' + port + '/', appConf.app)
        var serverIpUrl = Util.urlJoin('http://' + Util.getLocalIp() + ':' + port + '/', appConf.app)
        console.log('> Listening at ' + serveUrl)
        console.log('> Listening at ' + serverIpUrl)
        !appConf.noOpenBrowser && open(serveUrl)
      })

      // 监听不同类型文件的改变
      // 监听page的html文件变动
      
      watch(`${modulesGlobPrefix}/page/**/*.html`, function (ev) {
        var fPath = ev.path;
        var moduleFolder = Util.getModuleInfoViaPath(fPath, conf.appPath);
        var modulePath = path.join(conf.appPath, moduleFolder);
        var moduleInnerPath = fPath.replace(modulePath, '');
      ...
      // 监听widget的html文件变动
      watch(`${modulesGlobPrefix}/widget/**/*.html`, function (ev) {
        var fPath = ev.path;
        var moduleFolder = Util.getModuleInfoViaPath(fPath, conf.appPath);
        var modulePath = path.join(conf.appPath, moduleFolder);
        var moduleInnerPath = fPath.replace(modulePath, '');
      ...
      // 监听page的sass/less/css/js文件的变动
      watch([
        `${modulesGlobPrefix}/page/**/*.{scss,sass,less,css,js}`,
        `${modulesGlobPrefix}/page/*.{scss,sass,less,css,js}`
      ], function (ev) {
        var fPath = ev.path;
        var moduleFolder = Util.getModuleInfoViaPath(fPath, conf.appPath);
      ...
      // 监听widget的sass/less/css/js文件的变动
      watch([
        `${modulesGlobPrefix}/widget/**/*.{scss,sass,less,css,js}`,
        `${modulesGlobPrefix}/widget/*.{scss,sass,less,css,js}`
      ], function (ev) {
        var fPath = ev.path;
        var moduleFolder = Util.getModuleInfoViaPath(fPath, conf.appPath);
      ...
      watchExtraDirs(mod, conf)

      // 监听所有图片文件的变动
      watch(`${modulesGlobPrefix}/{page,static,widget}/**/images/**`, function (ev) {
        var fPath = ev.path;
        var moduleFolder = Util.getModuleInfoViaPath(fPath, conf.appPath);
        var modulePath = path.join(conf.appPath, moduleFolder);
        var moduleConf = require(path.join(modulePath, 'module-conf'));
      ...
      watch([
        `${modulesGlobPrefix}/static/**/*.{scss,sass,less,css,js}`,
        `${modulesGlobPrefix}/static/*.{scss,sass,less,css,js}`
      ], function (ev) {
        var fPath = ev.path;
        var moduleFolder = Util.getModuleInfoViaPath(fPath, conf.appPath);
    ...
    return buildModuleFunction(app, mod, conf, args)
      ...
      .then(function () {
        ...
        refModuleList.push(mod)
        refModuleList.push(common)
        const webpackBaseConfig = require('./webpack.base.config')(conf.appPath, modulePath, conf.appConf)
        const webpackDevConfig = require('./webpack.dev.config')(conf.appPath, modulePath, conf.appConf)
        const moduleOutputPath = path.join(modulePath, 'dist', 'output')
        const readOutputTpl = fs.readdirSync(path.join(moduleOutputPath, 'tpl'))
        const allPages = []
        let entry = {}
        ...
        const readStaticFolder = fs.readdirSync(path.join(moduleOutputPath, 's', 'static', 'js'))
        readStaticFolder.forEach(function (item) {
          if (Util.regexps.js.test(path.extname(item))) {
            const filename = path.basename(item, path.extname(item))
            const jsPath = path.join(moduleOutputPath, 's', 'static', 'js', filename + '.js')
            entry[filename] = [
              `webpack-dev-server/client?http://localhost:${port}/`,
              'webpack/hot/dev-server',
              jsPath
        ...
        fs.ensureDirSync(path.join(conf.appPath, '.temp', appConf.app, mod))
        webpackConfig = webpackMerge(webpackDevConfig , webpackBaseConfig, {
          entry: entry,
          output: {
            path: path.join(conf.appPath, '.temp', appConf.app, mod),
            filename: 'page/[name]/[name].js',
...
function publish (app, mod, args) {
  ...
  if (conf.buildType === BUILD_APP) {
    ...
    }, webpackDllPromise(null, null, conf.appPath))
      .then(function () {
        return Promise.resolve({
          appConf: appConf,
          files: publishFiles
