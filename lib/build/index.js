      ...
      .then(function () {
        ...
        if (staticConf && !_.isEmpty(staticConf.staticPath)) {
          vfs.watch([
            ...
            buildSingleModuleSimple(app, app, conf, args).then(function () {
              browserSync.reload();
            });
          });
        }
  ...
  if (conf.buildType === BUILD_APP) {
    ...
    return promsies.reduce(function (prev, curr) {
      return prev.then(function (val) {
        ...
        return buildSingleModule(app, moduleList[curr], conf, args).then(deployPromise.bind(null, moduleList[curr], modulePath, conf.appPath));
      });
    ...
    var deployPromise = taskList.deploy($, conf.appConf, conf.moduleConf);
    return buildSingleModule(app, mod, conf, args).then(deployPromise.bind(null, mod, conf.modulePath, conf.appPath))
      then(function () {
        return Promise.resolve({
          appConf: conf.appConf,
          mods: [mod]
