...
function serveServer (conf, app, mod, args) {
  ...
  if (conf.buildType === BUILD_APP) {
    return build(app, mod, _.assign({ isSimple: !isDist }, args)).then(function (moduleList) {
      ...
      watch(modules.map(v => `${v}/page/**/*.html`), function (ev) {
        var fPath = ev.path;
        var moduleFolder = Util.getModuleInfoViaPath(fPath, conf.appPath);
        var modulePath = path.join(conf.appPath, moduleFolder);
        var moduleInnerPath = fPath.replace(modulePath, '');
      ...
      // 监听widget的html文件变动
      watch(modules.map(v => `${v}/widget/**/*.html`), function (ev) {
        var fPath = ev.path;
        var moduleFolder = Util.getModuleInfoViaPath(fPath, conf.appPath);
        var modulePath = path.join(conf.appPath, moduleFolder);
        var moduleInnerPath = fPath.replace(modulePath, '');
      ...
      // 监听page的sass/less/css/js文件的变动
      watch(modules.reduce((pre, cur) => {
        return pre.concat([
          `${cur}/page/**/*.{scss,sass,less,css,js}`,
          `${cur}/page/*.{scss,sass,less,css,js}`
        ])
      }, []), function (ev) {
        var fPath = ev.path;
      ...
      watch(modules.reduce((pre, cur) => {
        return pre.concat([
          `${cur}/widget/**/*.{scss,sass,less,css,js}`,
          `${cur}/widget/*.{scss,sass,less,css,js}`
        ])
      }, []), function (ev) {
        var fPath = ev.path;
      ...
      watch(modules.reduce((pre, cur) => {
        return pre.concat([
          `${cur}/static/**/*.{scss,sass,less,css,js}`,
          `${cur}/static/*.{scss,sass,less,css,js}`
        ])
      }, []), function (ev) {
        var fPath = ev.path;
