...
function buildSingleModule( app, mod, conf, args ) {
  ...
  var sassPromise = taskList.sass( $, conf.appConf, conf.moduleConf );
  var lessPromise = taskList.less( $, conf.appConf, conf.moduleConf );
...
function serve( app, mod, args ) {
  ...
  if ( conf.buildType === BUILD_APP ) {
    ...
    del.sync( path.join( conf.appPath, '.temp', mod ), {
      force: true
    } );
