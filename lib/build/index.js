...
function serve(app, mod, args) {
  ...
  var isDist = args ? args.dist : false;
  if (conf.buildType === BUILD_APP) {
    del.sync(path.join(conf.appPath, '.temp'));
    build(app, mod, { isSimple: !isDist }).then(function () {
    ...
    vfs.watch([
      ...
      if (moduleFolder === conf.appConf.common) {
        build(app, null, { isSimple: !isDist }).then(function () {
          browserSync.reload();
        });
      } else {
        if (isDist) {
    ...
    buildModuleFunction(app, mod, conf)
