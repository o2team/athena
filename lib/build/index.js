...
function serveServer (conf, app, mod, args) {
  ...
  if (conf.buildType === BUILD_APP) {
    build(app, mod, _.assign({ isSimple: !isDist }, args)).then(function () {
      ...
      var serverParam = {
        baseDir: tempFolder,
        shortPath: conf.appConf.app
      };
      if (page && mod) {
        serverParam.baseDir = [tempFolder, path.join(tempFolder, mod)];
        serverParam.index = path.join(tempFolder, conf.appConf.app, mod, page + '.html');
      }
      var maltose = new Maltose({
        port: 35729,
        server: serverParam
...
function publish (app, mod, args) {
  ...
  if (conf.buildType === BUILD_APP) {
    ...
    var commonModule = conf.appConf.common;
