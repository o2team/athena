...
function getModuleInfoViaPath (fPath, appPath) {
...
function buildSingleModule (app, mod, conf, args) {
  ...
  del.sync(path.join(conf.modulePath, 'dist', '_static', 'css'));
  del.sync(path.join(conf.modulePath, 'dist', '_static', 'js'));
  del.sync(path.join(conf.modulePath, 'dist', 'output'));
  del.sync(path.join(conf.modulePath, 'dist', 'map.json'));
  ...
  var revImagesPromise = taskList.rev_images($, conf.appConf, conf.moduleConf, args);
  var replaceStaticPromise = taskList.replace_static($, conf.appConf, conf.moduleConf, args);
  ...
  return allPromise(mod, conf.modulePath, conf.appPath)
    ...
    .then(revImagesPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(fetchCommonPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(replaceStaticPromise.bind(null, mod, conf.modulePath, conf.appPath))
...
function buildSingleModuleServer (app, mod, conf, args) {
  ...
  conf = getConf(app, mod);
  del.sync(path.join(conf.modulePath, 'dist', '_static', 'page'));
  del.sync(path.join(conf.modulePath, 'dist', '_static', 'widget'));
  del.sync(path.join(conf.modulePath, 'dist', '_static', 'static'));
  del.sync(path.join(conf.modulePath, 'dist', 'output'));
  del.sync(path.join(conf.modulePath, 'dist', 'map.json'));
  var allPromise = taskList.all($, conf.appConf, conf.moduleConf, args);
  var sassPromise = taskList.sass($, conf.appConf, conf.moduleConf, args);
  ...
  return allPromise(mod, conf.modulePath, conf.appPath)
    ...
    .then(revServerPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(fetchCommonPromise.bind(null, mod, conf.modulePath, conf.appPath))
...
function buildSingleModuleSimpleServer (app, mod, conf, args) {
  ...
  var stylesServerPromise = taskList.styles_server($, conf.appConf, conf.moduleConf, args);
  var injectServerPromise = taskList.inject_server($, conf.appConf, conf.moduleConf, args);
  var transOutputPromise = taskList.trans_output($, conf.appConf, conf.moduleConf, args);
  var tempServerPromise = taskList.temp_server($, conf.appConf, conf.moduleConf, args);
  $.util.log($.util.colors.green('开始编译模块' + mod + '！'));
  return allPromise(mod, conf.modulePath, conf.appPath)
    .then(sassPromise.bind(null, mod, conf.modulePath, conf.appPath))
    ...
    .then(stylesServerPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(injectServerPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(transOutputPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(tempServerPromise.bind(null, mod, conf.modulePath, conf.appPath))
    .then(function (result) {
      $.util.log($.util.colors.green('结束编译模块' + mod + '！'));
      return Promise.resolve(result);
...
function build (app, mod, args) {
  ...
  if (comboConf.mode === 'client') {
    ...
    return buildCheck(app, mod, conf, args, buildSingleModule)
      .then(generateIncludePromise.bind(null, mod, conf.modulePath, conf.appPath))
      .catch(function (e) {
        console.log(e.stack);
      });
  }
...
function serve(app, mod, args) {
  ...
  args = _.assign(args, { isServe: true });
  if (comboConf.mode === 'client') {
  ...
  if (conf.buildType === BUILD_APP) {
    build(app, mod, _.assign({ isSimple: !isDist }, args)).then(function () {
      ...
      if (page && mod) {
        ...
        serverParam.index = path.join(tempFolder, appConf.app, mod, page + '.html');
      }
      ...
      vfs.watch([
        ...
        var fPath = ev.path;
        var moduleFolder = getModuleInfoViaPath(fPath, conf.appPath);
    ...
    buildModuleFunction(app, mod, conf, args)
      ...
      .then(function () {
        ...
        if (page) {
          serverParam.baseDir = [tempFolder, tempFolder + '/' + conf.moduleConf.module];
          serverParam.index = conf.moduleConf.module + '/' + page + '.html'
        }
...
function publish (app, mod, args) {
  ...
  if (conf.buildType === BUILD_APP) {
    ...
    return buildModuleFunction(app, mod, conf, args)
      .then(uploadPromise.bind(null, mod, modulePath, conf.appPath))
      .then(publishPromise.bind(null, mod, modulePath, conf.appPath))
      .then(function (files) {
        return Promise.resolve({
