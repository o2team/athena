...
function buildSingleModuleSimple (app, mod, conf, args) {
  ...
  var serveTransPromise = taskList.serve_trans($, conf.appConf, conf.moduleConf);
...
function serve(app, mod, args) {
  ...
  if (conf.buildType === BUILD_APP) {
    del.sync(path.join(conf.appPath, '.temp'));
    build(app, mod, { isSimple: true }).then(function () {
    ...
    buildSingleModuleSimple(app, mod, conf)
      ...
      .then(function () {
        ...
        // 监听page的sass/less/css文件的变动
        vfs.watch([
          'page/**/*.?(css|sass|scss|less)',
        ], function (ev) {
          var fPath = ev.path;
          ...
          var args = { pageFiles: [pageFile], cssFile: path.join(fileDest, fileName) };
          var copyFilePromise = taskList.copy_file($, conf.appConf, conf.moduleConf, {
            src: fPath,
            dest: fileDest
          });
          console.log(path.join(fileDest, fileName));
          var serveCssPromise = taskList.serve_css($, conf.appConf, conf.moduleConf, args);
          copyFilePromise(mod, conf.modulePath, conf.appPath)
            .then(serveCssPromise.bind(null, mod, conf.modulePath, conf.appPath))
            .then(function () {
              browserSync.reload();
            });
        });
      ...
      // 监听widget的sass/less/css文件的变动
      vfs.watch([
          'widget/**/*.?(css|sass|scss|less)',
        ], function (ev) {
