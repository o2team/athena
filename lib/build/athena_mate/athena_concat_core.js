...
var commentReg  = /(?:\/\*(?:[\s\S]*?)\*\/)|(?:([\s;])+\/\/(?:.*)$)/gm;

function concatCore (file, opt) {
  if (!file) {
    throw new PluginError('athena_concat_core', '必需传入文件');
  }
  opt = opt || {};
  ...
  if (typeof file === 'string') {
    ...
    throw new PluginError('athena_concat_core', '未知文件');
  }
  ...
  var stream = through(function (file, encoding, callback) {
    ...
    if (file.isStream()) {
      this.emit('error', new PluginError('athena_concat_core',  '不支持Streaming'));
      callback();
      return;
    }
    ...
    contentStr = contentStr.replace(commentReg, '').replace(/(^\s+)|(\s+$)/g, '');
