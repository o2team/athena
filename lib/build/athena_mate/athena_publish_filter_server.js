...
module.exports = function (opts) {
  ...
  return through2.obj(function (file, encoding, cb) {
    ...
    var fpath = file.path;
    fpath = Util.getStaticPathServer(fpath, path.join(config.cwd, config.module, 'dist', 'output')).path;
    if (fpath.indexOf('\\') >= 0) {
    ...
    var publishDataFilePath = path.join(publishDataFileDir, config.remote + '_server_filter.json');
    ...
    fileNameList.forEach(function (item, i) {
      console.log('========' + item);
      var revName = Util.getHashNameServer(item, mapJson);
      console.log('========' + revName);
      if (fileNameList.indexOf(revName) >= 0 && revName !== item) {
        newFileNameList[item] = revName;
      }
    });
    console.log(newFileNameList);
    ...
    fileList.forEach(function (item) {
      var name = Util.getStaticPathServer(item.path, path.join(config.cwd, config.module, 'dist', 'output')).path;
      if (name.indexOf('\\') >= 0) {
        name = name.replace(/\\/g, '\/');
      }
