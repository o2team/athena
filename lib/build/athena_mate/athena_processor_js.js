...
function trackFilePath (depPath, opts) {
  var depFilePath = ''
...
function transfromSyncDep (dep, regexp, line, needDelete, opts) {
  ...
  depFilePath = trackFilePath(depPath, opts)
...
function transfromLazyDep (dep, regexp, line, opts) {
  var depPath = dep['path']
  var arg = dep['variable']
  ...
  return through2.obj(function (file, enc, cb) {
    ...
    if (opts.type === 'nerv') {
      ...
      content = trackJsDepAndModify(content, function (type, line, dep) {
        if (type === 'sync' && dep) {
          var transfromDepRes = transfromSyncDep(dep, Util.regexps.require, line, needDelete, opts)
          var lineRes = transfromDepRes.line
          delete transfromDepRes.line
          syncDeps.push(transfromDepRes)
          return lineRes
        } else if (type === 'lazy' && dep) {
          var transfromDepRes = transfromLazyDep(dep, Util.regexps.requireAsync, line, opts)
          var lineRes = transfromDepRes.line
          delete transfromDepRes.line
