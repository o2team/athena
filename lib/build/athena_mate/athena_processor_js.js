...
function directoryUp (dir, name, cb, disableCache) {
  var i, cache, dirs, filePath, result;

  dirs = dir.split(path.sep).map(function(d, i, ref) {
    return ref.slice(0, i + 1).join(path.sep);
  });
...
function getPkg (dir) {
  return directoryUp(dir, 'package.json', function(filePath, dir) {
    const pkg = Util.readJsonFile(filePath)
    let result
...
function trackFilePath (fpath, depPath, opts) {
  ...
  if (depPath.indexOf('APP_ROOT') >= 0) {
    ...
    // 解析node_modules资源
    var cacheRootPath = path.join(opts.appPath, Util.CACHE)
    var cachePath = path.join(cacheRootPath, 'nodeModules')
    var cacheFilePath = path.join(cachePath, 'cache.json')
    if (!Util.existsSync(cachePath)) {
      mkdirp.sync(cachePath);
    }
    var cachePathJson = Util.readJsonFile(cacheFilePath)
    try {
      const mainPath = resolve.sync(depPath, {
        basedir: opts.appPath
      })
      const depPkg = getPkg(path.dirname(mainPath))
      if (!cachePathJson[depPath] || cachePathJson[depPath]['version'] !== depPkg.version) {
        cachePathJson[depPath] = depPkg
        fs.writeFileSync(cacheFilePath, JSON.stringify(cachePathJson, null, 2))
      }
      const mainPathStr = String(fs.readFileSync(mainPath))
    } catch (err) {
      console.log(err)
    }
  }
