...
function transfromDep (dep, regexp, line, needDelete, opts) {
  ...
  const arg = dep['variable']
  let depFilePath = depPath
  if (depPath.indexOf('APP_ROOT') >= 0) {
    depFilePath = depPath.replace('APP_ROOT', '')
    depFilePath = path.join(opts.appPath, depFilePath)
  }
  if (depPath.indexOf('MOD_ROOT') >= 0) {
    depFilePath = depPath.replace('MOD_ROOT', '')
    depFilePath = path.join(opts.modulePath, depFilePath)
  }
  const extname = path.extname(depFilePath)
  if (!extname) {
    depFilePath += '.js'
  }
  if (Util.existsSync(depFilePath)) {
    const depFilePathInner = depFilePath.replace(path.join(opts.appPath, '/'), '')
    const depFilePathInnerArr = depFilePathInner.split(path.sep)
    const depModuleName = depFilePathInnerArr[0]
    const depDirName = depFilePathInnerArr[1]
    const depName = depFilePathInnerArr[2]
    const realDep = depModuleName + '/' + depDirName + '/' + depName
    // 替换掉当前依赖
...
module.exports = function (opts) {
  return through2.obj(function (file, enc, cb) {
    ...
    if (opts.type === 'nerv') {
      ...
      let syncDeps = []
      let asyncDeps = []
      let needDelete = fileInnerFirstDir === 'page'
      content = trackJsDepAndModify(content, function (type, line, dep) {
        if (type === 'sync' && dep) {
          const transfromDepRes = transfromDep(dep, Util.regexps.require, line, needDelete, opts)
          const lineRes = transfromDepRes.line
          delete transfromDepRes.line
          syncDeps.push(transfromDepRes)
          return lineRes
        } else if (type === 'async' && dep) {
          const transfromDepRes = transfromDep(dep, Util.regexps.requireAsync, line, needDelete, opts)
          const lineRes = transfromDepRes.line
          delete transfromDepRes.line
      ...
      if (fileInnerFirstDir === 'page') {
        let depStr = ''
        let argsStr = ''
        const allDeps = syncDeps.concat(asyncDeps)
        if (allDeps.length) {
          depStr = '['
