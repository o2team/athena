...
function printResource (filename, value, resourcePrefix, config, includeJson) {
  ...
  if (Util.regexps.url.test(value)) {
    ...
    var fdir = path.join(athenaPath, 'cache', 'remote', 'files');
    if (!Util.existsSync(fdir)) {
      mkdirp.sync(fdir);
    }
    fpath = path.join(fdir, Util.checksum(value, 16));
  ...
  } else if (Util.regexps.tpl.test(value)) {
