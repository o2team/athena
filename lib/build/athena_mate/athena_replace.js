  ...
  if (config.appConf && config.moduleConf) {
    ...
    for (var key in deployObj) {
      if (key === 'local') {
        ...
        resourcePrefix[key] = '//' + Util.urlJoin(deployObj[key].domain, deployObj[key].fdPath, config.appConf.app, '/');
      }
    }
  }
...
function transmitResource(filename, value, resourcePrefix, config, includeJson, needMd5) {
  ...
  if (!Util.regexps.url.test(value) && value.length > 0
    ...
    var pathname = path.join(path.join(config.cwd, moduleName, 'dist/output', originalPath), '..', path.basename(value)),
      size;
    if (Util.regexps.images.test(path.basename(pathname)) && config.base64Opts.enable) {
      // 判断是否被excluded
      if (_.isArray(config.base64Opts.exclude) && config.base64Opts.exclude.length > 0 && config.base64Opts.exclude.indexOf(originalPath) >= 0);
      else {
        try {
          size = fs.statSync(pathname).size;
          if (size < config.base64Opts.size) {
            return Util.transform2DataURI(pathname);
          }
        }
        catch (e) {
          // 未找到文件
  ...
  value = path.normalize(vStart + value + vEnd).replace(/['"']/g, '');
  return value;
}
