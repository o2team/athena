...
function transmitResource (filename, value, resourcePrefix, needMd5) {
  ...
  if (!Util.regexps.url.test(value) && value.length > 0 && imagesIndex >= 0) {
    value = trackResource(value, imagesIndex);
    value = value.replace(/\\/ig,'/');
    var moduleName = value.split('/')[0]; //这里还是要改成'/' 不能用path.sep 因为href的中的css链接都是'/'
    if (moduleName !== config.module && moduleName !== 'data:image') {
...
function trackResource (value, imagesIndex) {
