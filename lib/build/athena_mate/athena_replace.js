        ...
        m = m.replace(/(\s(?:data-)?src(?:set)?\s*=\s*)('[^']+'|"[^"]+"|[^\s\/>]+)/ig, function(m, prefix, value) {
          if (prefix.indexOf('srcset') != -1) {
            ...
            info.rest.split(',').forEach(function(item) {
              ...
              val = getHashName(val, mapJson);
              srcset.push(resourcePrefix[config.replaceType] + val);
            });
  ...
  contents = contents.replace(reg, function (m, comment, url, last, filter) {
    ...
    if (url) {
      if (m.indexOf('@') === 0) {
        ...
        url = getHashName(url, mapJson);
        m = 'url(' + resourcePrefix[config.replaceType] + url + ')' + last;
      }
    } else if (filter) {
      var imagesIndex = filter.indexOf('images');
      if (imagesIndex) {
        filter = filter.substr(imagesIndex);
      }
      filter = filter.substr(imagesIndex);
      filter = getHashName(filter, mapJson);
      m = 'src=' + resourcePrefix[config.replaceType] + filter;
    }
    return m;
  });
  return contents;
