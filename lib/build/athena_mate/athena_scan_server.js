      ...
      if (fs.existsSync(widgetPath)) {
        try {
          ...
          if (Util.existsSync(tplPath)) {
            var widgetBuf = fs.readFileSync(tplPath);
            var widgetContent = String(widgetBuf);
            var floorIdReg = /{[\s\S]+?$portal_floor_id[\s\S]+?}/g;
            ...
            // 去掉{literal}{/literal}及之间不需要关注的代码，同时解析出各个cell对应的数据
            widgetContent = widgetContent.replace(literalReg, '')
              .replace(Util.regexps.doubleBraceInterpolate, function (m, $1, $2) {
