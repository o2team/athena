...
function inject (opts) {
  ...
  if (!config.serve && !_.isEmpty(config.useInclude.files)) {
    ...
    for (var i in includeFiles) {
      ...
      fileChunk[i] = {content: [], module: includeFiles[i].module};
      fileChunk[scriptFile] = {content: [], module: includeFiles[i].module};
    }
  ...
  var stream = through2.obj(function (file, encoding, callback) {
    ...
    if (!config.serve && !_.isEmpty(config.useInclude.files)) {
      ...
      for (var i in fileChunk) {
        var includeFolder = path.join(config.cwd, fileChunk[i].module, 'dist', 'output', useInclude.folder);
          ...
          if (Util.existsSync(path.join(widgetPath, widgetName + '.html'))) {
            ...
            if (!_.isEmpty(ViewHelper.config.useInclude.useIncludeWidgetList)) {
              ...
              useIncludeWidgetList.forEach(function (item) {
                if (item.widgetName === widgetName && item.module === moduleName) {
                  var extname = path.extname(item.file);
                  var scriptFile = path.basename(item.file, extname) + '_script' + extname;
                  fileChunk[item.file].content.push(widgetHtmlStr);
                  fileChunk[scriptFile].content.push(scriptContent);
                  scriptContent = '<!--#include file="' + Util.urlJoin(useInclude.pathPrefix, scriptFile) + '"-->';
                  widgetHtmlStr = '<!--#include file="' + Util.urlJoin(useInclude.pathPrefix, item.file) + '"-->';
                }
              });
            }
