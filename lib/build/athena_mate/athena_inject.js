...
var ViewHelper = {
  ...
  getCSS: function (resourceName, moduleName) {
    ...
    if (!mapJSONCache[moduleName]) {
      mapJSONCache[moduleName] = JSON.parse(fs.readFileSync(path.join(ViewHelper.config.cwd, moduleName, 'dist', 'map.json')).toString());
    }
  ...
  widget: {
    ...
    load: function (widgetName, param, moduleName) {
      ...
      param = param ? param : {};
      var modulePath = ViewHelper.config.cwd + '/' + moduleName;
