...
function getInlineStr (config, appConf, hrefText) {
  ...
  if (resPath.split('/')[0] === appConf.common) {
    resPath = resPath.replace(appConf.common, '');
    resPath = path.join(config.cwd, appConf.common, 'dist', 'output', resPath);
  } else {
    resPath = resPath.replace(config.module, '');
    resPath = path.join(config.cwd, config.module, 'dist', 'output', resPath);
  }
