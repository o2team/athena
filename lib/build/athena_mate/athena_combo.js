...
function generateShtml ($, fileName, config, appConf, resType, dom, tag, uriType) {
  ...
  if (config.needCombo) {
    ...
    if (dom.length - inlineDomLength <= 1) {
      href = '\/\/' + config.domain;
    }
...
function combo (opts) {
  ...
  var stream = through2.obj(function (file, encoding, callback) {
    ...
    if (file.isBuffer()) {
      ...
      this.push(file);
      var cssCommentStr = '<!-- #include virtual="' + Util.urlJoin(config.shtmlPrefix, config.module, fileName + '.shtml') + '" -->\n';
      var jsCommentStr = '<!-- #include virtual="' + Util.urlJoin(config.shtmlPrefix, config.module, fileName + '_js.shtml') + '" -->\n';
