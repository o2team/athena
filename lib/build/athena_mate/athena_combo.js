...
function combo (opts) {
  ...
  var stream = through(function (file, encoding, callback) {
    ...
    if (file.isBuffer()) {
      ...
      var combofile = opts.fdPath + '/' + opts.app + '/' + opts.module + '/' + fileName + '.shtml';
      var fileContent = file.contents.toString();
      var fileList = [];
      var href = 'http://' + opts.domain + '/c/=';
      var newFileContent = '';
      ...
      links.each(function (i, item) {
        var combouse = opts.fdPath + '/' + opts.app + item['attribs']['combo-use'];
        href += combouse;
        if (i === links.length - 1) {
          href += '?t=' + parseInt(new Date().getTime() / 1000, 10);
        } else {
          href += ',';
        }
      });
