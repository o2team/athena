...
function generateShtml ($, fileName, config, appConf, resType, dom, tag, uriType) {
  ...
  if (config.needCombo) {
    ...
    var failoverHref
    if (failoverDomain) {
      failoverHref = '\/\/<!--#echo var=\'http_x_failover_domain\' -->' + config.comboPrefix
    }
    ...
    if (lenCount === 1) {
      href = '\/\/' + config.domain;
      if (failoverDomain) {
        failoverHref = '\/\/<!--#echo var=\'http_x_failover_domain\' -->'
      }
    }
...
function combo (opts) {
  ...
  var stream = through2.obj(function (file, encoding, callback) {
    ...
    if (file.isBuffer()) {
      ...
      var cssShtmlObj = generateShtml($, fileName, config, appConf, 'css', links, 'link', 'href')
      ...
      if (canFailover) {
        cssFileContent += '<!--#if expr="$http_x_failover_domain" -->\n'
