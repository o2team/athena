...
function generateShtml ($, fileName, config, appConf, resType, dom, tag, uriType) {
  ...
  if (config.needCombo) {
    ...
    var template = '';
    if (tag === 'link') {
      template = '<link combofile="<%= combofile %>" rel="stylesheet" href="<%= href %>" onerror="__reloadResource" />';
    } else {
      template = '<script combofile="<%= combofile %>" src="<%= href %>" onerror="__reloadResource"></script>';
    }
    content += inlineStr;
    if (lenCount > 0) {
    ...
    dom.each(function (i, item) {
      ...
      if (_.isUndefined(inline)) {
        ...
        var template = '';
        if (tag === 'link') {
          template = '<link rel="stylesheet" href="<%= href %>" onerror="__reloadResource" />';
        } else {
          template = '<script src="<%= href %>" onerror="__reloadResource"></script>';
        }
