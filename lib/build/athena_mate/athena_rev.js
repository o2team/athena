...
function rev (opts) {
  ...
  var stream = through2.obj(function (file, encoding, callback) {
    ...
    if (file.isBuffer()) {
      ...
      moduleInnerPath = moduleInnerPath.split(path.sep);
      moduleInnerPath = moduleInnerPath.join('/');
      var imagesIndex = moduleInnerPath.indexOf('images');
      var jsIndex = moduleInnerPath.indexOf('js');
      var cssIndex = moduleInnerPath.indexOf('css');
      var index = Math.max(imagesIndex, jsIndex, cssIndex);
      if (index >= 0) {
        filename = moduleInnerPath.substr(index);
      }
