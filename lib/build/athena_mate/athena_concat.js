    ...
    fs.readFile(path.join(modulePath, config.map), function (err, data) {
      ...
      for (var key in conf) {
        ...
        if (conf.hasOwnProperty(key)) {
          ...
          var include = JSON.parse(fileContent).include[key];
          ...
          console.log(includeCommonWidgets);
...
function concactStatic (modulePath, config, cb) {
  ...
  streamArr.push(vinylFs
    .src([path.join(modulePath, config.dest, '_', 'static', '**', '*.css'), path.join(modulePath, config.dest, '_', 'static', 'sass', '**', '*.css')])
    .pipe(vinylFs.dest(path.join(modulePath, config.dest, '_static')))
  );
  streamArr.push(vinylFs
    .src(path.join(modulePath, config.dest, '_', 'static', '**', '*.js'))
    .pipe(vinylFs.dest(path.join(modulePath, config.dest, '_static')))
  );
  es.merge(streamArr).on('end', function () {
    var streamArr = [];
