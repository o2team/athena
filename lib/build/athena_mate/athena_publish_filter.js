...
module.exports = function (opts) {
  ...
  return through2.obj(function (file, encoding, cb) {
    ...
    fileNameList.push(path.basename(file.path));
    fileList.push(file);
    cb();
  }, function (cb) {
    var athenaPath = Util.getAthenaPath();
    var publishDataFileDir = path.join(athenaPath, 'publish', config.app);
    var publishDataFilePath = path.join(publishDataFileDir, config.module + '_filter.json');
