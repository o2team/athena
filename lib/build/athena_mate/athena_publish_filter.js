...
function getStaticPath (fpath) {
  fpath = fpath.split(path.sep);
  fpath = fpath.join('/');
  var imagesIndex = fpath.indexOf('images');
  var jsIndex = fpath.indexOf('js');
  var cssIndex = fpath.indexOf('css');
  var index = Math.max(imagesIndex, jsIndex, cssIndex);
  if (index >= 0) {
    fpath = fpath.substr(index);
  }
  return fpath;
}
...
module.exports = function (opts) {
  ...
  return through2.obj(function (file, encoding, cb) {
    ...
    fileList.forEach(function (item) {
      var name = getStaticPath(item.path);
