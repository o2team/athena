...
function imagemin (opts) {
  ...
  var stream = through2.obj(function (file, encoding, callback) {
    ...
    if (file.isBuffer()) {
      if (realValidExts.indexOf(extname.toLowerCase()) === -1) {
        return callback(null, file);
      }
      ...
      var extension = extname.replace(/^\./, '')
      var imageConf = opts[extension]
      var outputFilePath = path.join(tempy.file({extension: extname.replace(/^\./, '')}))
      var compressConf = {
        input: file.path,
