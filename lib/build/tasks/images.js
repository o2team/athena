        ...
        .pipe(athenaMate.buildFilter({
          ...
          checkCb: function (item) {
            ...
            if (Util.existsSync(fpath)) {
              md5Hash = Util.checksum(fs.readFileSync(fpath), 16);
            }
            var imagesMinRev = {};
            var cacheRevPath = path.join(Util.getAthenaPath(), 'cache', 'build', 'images', appConf.app, moduleConf.module, 'cache_rev.json');
            if (!Util.existsSync(cacheRevPath)) {
              return true;
            }
            try {
              imagesMinRev = JSON.parse(fs.readFileSync(cacheRevPath));
            } catch (e) {
              console.log(e);
            }
            return imagesMinRev[item.path] !== md5Hash;
          }
        }))
