        ...
        if (args.needGraphCheck) {
          if (moduleName !== appConf.common) {
            ...
            appConf.moduleList.forEach(function (item) {
              ...
              appSassCacheJSon[item] = Util.readJsonFile(path.join(Util.getAthenaPath(), 'cache', 'build', 'sass', appConf.app, item, 'cache.json'));
            });
            ...
            for (var i in appSassCacheJSon) {
              if (changedModules.indexOf(i) >= 0) {
                fs.writeFileSync(path.join(Util.getAthenaPath(), 'cache', 'build', 'sass', appConf.app, i, 'cache.json'), JSON.stringify(appSassCacheJSon[i], null, 2));
              }
            }
          }
        }
