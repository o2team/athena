...
module.exports = function ($, appConf, moduleConf, args) {
  return function (mod, modulePath, appPath, remoteName) {
    return new Promise(function (resolve, reject) {
      ...
      var useHash = support.useHash ? support.useHash.enable === undefined ? true : support.useHash.enable : true;
      ...
      inquirer.prompt(prompt, function (answers) {
        ...
        if (answers.pages) {
          ...
          filterPages.forEach(function (item) {
            ...
            var combofileHtmlPath = path.join(cpath, item);
          ...
          if (deployParams.mode === 'http') {
            vfs.src(globPages.concat(htmlPathList), { base: path.join(modulePath, 'dist', 'output') })
              .pipe(athenaMate.if(useHash ? true : false, athenaMate.publishFilter({
                cwd: appPath,
                app: appConf.app,
              ...
              .on('end', function () {
                vfs.src(combofileSHtmlList)
                  .pipe(athenaMate.if(useShtml, athenaMate.jdcFinder({
          ...
          if (parseInt(deployRemoteParams.port, 10) === 21) {
            vfs.src(globPages.concat(htmlPathList), { base: path.join(modulePath, 'dist', 'output') })
              ...
              .on('finish', function () {
                vfs.src(combofileSHtmlList)
                  .pipe(athenaMate.if(useShtml, athenaMate.ftp(deployCssiParams)))
                  .on('data', function () {})
                  .pipe($.util.noop())
                  .on('finish', function () {
                    vfs.src(summaryPage, { base: path.join(appPath, '.temp', appConf.app) })
            ...
            vfs.src(globPages.concat(htmlPathList), { base: path.join(modulePath, 'dist', 'output') })
              ...
              .on('finish', function () {
                vfs.src(combofileSHtmlList)
                  .pipe(athenaMate.if(useShtml, gulpSSH.dest(deployCssiParams.remotePath)))
                  .on('finish', function () {
                    vfs.src(summaryPage, { base: path.join(appPath, '.temp', appConf.app) })
